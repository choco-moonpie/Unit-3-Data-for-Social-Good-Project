...
Our user story starts with a book data set, as book enthusiasts, Bella and I wanted to promote and bring awarness to the closing of librarbies and book stores, which is a major issue in today's society. Our code implements text files, which transverse data instead of an initializer list, using a nested loop to help fellow readers determine what books to read next. We coded a method called minRating, which has a condition within the nest loop,method increases the index everytime the loop is run. As a result, readers can find books with a rating of 4.20 or higher, which are printed into the console with a print statement. 